...
<div class="card">
    <div class="card-body">
        <div class="button-actions">
            <button type="button" class="btn btn-primary mb-2" data-bs-toggle="modal" data-bs-target="#modal_create"><i class="mdi mdi-plus-circle"></i> Thêm mới đánh giá thùng</button>
        </div>
        <table id="table_manage" data-action="{{route('admin.vat_report.get_data')}}" class="table dt-responsive w-100">
            <thead>
                <tr>
                    <th>
                    ...
                    <th>
                        <div class="text-uppercase align-middle">Mã đánh giá</div>
                    </th>
                    <th>
                        <div class="text-uppercase align-middle">Mã thùng</div>
                    </th>
                    <th>
                        <div class="text-uppercase align-middle">Thời gian đánh giá</div>
                    </th>
                    <th>
                        <div class="text-uppercase align-middle">Chỉ tiêu đánh giá</div>
                    </th>
                    <th>
                        <div class="text-uppercase align-middle">Người đánh giá</div>
                    </th>
                    <th>
...
<div class="list-modal">
    ...
    @include('admin.vat_report.modals.edit')
...
@push('js')
    <script>
        ...
        const elements = {
            ...
            action_update: @json(route('admin.vat_report.update', ['id' => ':id'])),
            action_delete: @json(route('admin.vat_report.delete', ['id' => ':id'])),
        }
                ...
                success: function(res){
                    if(res.success) {
                        $.each(res.data, function(key, value){
                            ...
                            }else if(elements.modal_edit.find('#'+key).hasClass('number')) {
                                elements.modal_edit.find('#'+key).val(value).trigger('blur');
                            }else {
        ...
        function updateStore(url, form, button_click){
            let object = {
                ...
                vat_id: form.find('#vat_id').val(),
                protein_level: form.find('#protein_level').val(),
                salt_level: form.find('#salt_level').val(),
                histamine_level: form.find('#histamine_level').val(),
                acid_level: form.find('#acid_level').val(),
                amon_level: form.find('#amon_level').val(),
        ...
        function renderTable(search){
            elements.table_manage.DataTable({
                ...
                columns: [
                    ...
                    { data: 'vat_code', name: "vat_code", class: 'align-middle'},
                    { data: 'evaluation_date', name: "evaluation_date", class: 'align-middle'},
                    { data: 'chi_tieu_dg', class: 'align-middle'},
