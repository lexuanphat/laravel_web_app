...
<div class="card mt-2">
    <div class="card-body">
        ...
        <div class="col">
            ...
            <input type="text" id="searchInput" class="form-control" placeholder="Tìm kiếm tên nhân viên">
            </div>
        </div>
...
<div class="card">
    <div class="card-body">
        <div class="button-actions">
            <button type="button" class="btn btn-primary mb-2" data-bs-toggle="modal" data-bs-target="#modal_create"><i class="mdi mdi-plus-circle"></i> Thêm mới nhân viên</button>
        </div>
        <table id="table_manage" data-action="{{route('admin.staff.get_data')}}" class="table dt-responsive w-100">
            <thead>
                <tr>
                    <th>
                    ...
                    <th>
                        <div class="text-uppercase align-middle">Tên nhân viên</div>
                    </th>
                    <th>
                        <div class="text-uppercase align-middle">Số điện thoại</div>
                    </th>
                    <th>
                        <div class="text-uppercase align-middle">Địa chỉ email</div>
                    </th>
                    <th>
        ...
        <div class="list-modal">
            @include('admin.staff.modals.create')
...
@push('js')
    <script>
        const elements = {
            ...
            action_update: @json(route('admin.staff.update', ['id' => ':id'])),
            action_delete: @json(route('admin.staff.delete', ['id' => ':id'])),
        }
        ...
        function getDataRecord(action, id, $this) {
            $.ajax({
                ...
                beforeSend: function(){
                    ...
                    $this.find('i').hide();
                    elements.modal_edit.find('input[type="password"]').val("");
                },
                success: function(res){
                    if(res.success) {
                        ...
                        if(res.hide_password_field) {
                            elements.modal_edit.find('input[type="password"]').parent().hide();
                        } else {
                            elements.modal_edit.find('input[type="password"]').parent().show();
                        }
