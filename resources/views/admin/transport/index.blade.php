...
@section('content')
    <div class="bg-white p-2 my-2">
        ...
        <table id="table_manage" data-action="{{route('admin.transport.get_data')}}" class="table dt-responsive w-100">
            <thead>
                <tr>
                    <th>
...
@push('js')
    <script>
        const elements = {
            ...
            action_update: @json(route('admin.transport.update', ['id' => ':id'])),
            action_delete: @json(route('admin.transport.delete', ['id' => ':id'])),
        }
                ...
                success: function(res){
                    if(res.success) {
                        $.each(res.data, function(key, value){
                            ...
                            if(key === 'role') {
                                elements.modal_edit.find('[type="radio"]').prop('checked', false);
                                elements.modal_edit.find(`[type="radio"][value="${value}"]`).prop('checked', true);
                            } else {
